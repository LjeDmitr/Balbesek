...
int rk_mytermregime(int regime, int vtime, int vmin, int echo, int sigint)
{
	...
	if (tcgetattr(STDIN_FILENO, &options) != 0) // Получает параметры терминала
	{
		return -1;
	}
	if (regime == 1)
	{
		options.c_lflag |= ICANON; // Включает канон режим
	}
	else if (regime == 0)
	{
		options.c_lflag &= ~ICANON; // Выключает канон режим
	}
	else
	{
		return -1;
	...
	if (regime == 0)
	{
		...
		if (echo == 1)
		{
			options.c_lflag |= ECHO; // Показывает вводимые символы
		}
		else if (echo == 0)
		{
		...
		if (sigint == 1)
		{
			options.c_lflag |= ISIG; // все вводимые символы сравниваются с символами INTR, QUIT, SUSP и DSUSP.
		}
		else if (sigint == 0)
		{
	...
	if (tcsetattr(STDIN_FILENO, TCSANOW, &options) != 0) // Меняет параметры терминала
	{
		return -1;
	}
